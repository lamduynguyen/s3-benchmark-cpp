...
inline auto TscDifferenceNs(uint64_t start, uint64_t end) -> uint64_t { return (end - start) / TSC_PER_NS; }

inline auto TscDifferenceUs(uint64_t start, uint64_t end) -> double { return TscDifferenceNs(start, end) / 1000.0; }

inline auto TscDifferenceMs(uint64_t start, uint64_t end) -> double { return TscDifferenceNs(start, end) / 1000000.0; }

inline auto TscDifferenceS(uint64_t start, uint64_t end) -> double {
  return TscDifferenceNs(start, end) / 1000000000.0;
}
